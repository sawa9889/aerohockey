...
UIcontainer = Class {
    init = function(self, x, y, width, height,columns, rows, margin)
        ...
        self.toolBar = { buttonLeft =  Button(self.x, self.y + self.height + self.margin, function(container) container:changePage(-1) print('left') end , 'Left'),
                         buttonRight = Button(self.x + self.width-100, self.y + self.height + self.margin, function(container) container:changePage(1) print('right')end, 'Right')    
                       }
    end
}
